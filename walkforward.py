...
# Import strategy classes here; add more as needed for new strategies
from deepseek_strategy import Intraday15MinStrategy

# Map strategy dropdown value to class
STRATEGY_MAP = {
...
class WalkForwardBacktester:
    def __init__(self, strategy_key, interval, symbol, capital=10000, param_optimization=True, param_grid=None):
        """
        strategy_key: string as selected from dropdown (e.g., "deep.boll.vwap.rsi.macd")
        interval: "5min", "15min", or "1hour"
        symbol: symbol string
        capital: starting capital per test
        param_optimization: whether to fit/optimize params on train set
        param_grid: grid for optimization
        """
    ...
    def load_data(self):
        """
        Get resampled data for this symbol and interval from resampler.
        Returns a DataFrame with columns ['timestamp','open','high','low','close','volume', ...]
        """
        data_list = resampler.get_resampled_data(self.interval)
        # Filter for the selected symbol (if not already filtered)
    ...
    def walkforward_splits(self, df, train_days=90, test_days=30):
        """
        Yield (train_df, test_df, train_start, test_start) for each walk-forward window.
        """
        ...
        while True:
            ...
            # Check if test window goes beyond available data
            ...
            walk_start = test_start

    def run_backtest(self):
        """
        Main orchestration method.
        Returns: results dict with stats, trades, and per-window info.
        """
        df = self.load_data()
        ...
        for idx, (train_df, test_df, train_start, test_start) in enumerate(windows):
            # Strategy is reinitialized for each window
            strategy = self.strategy_cls()
            best_params = None
            best_stats = None
            if self.param_optimization and hasattr(strategy, "fit"):
                best_params, best_stats = strategy.fit(train_df, param_grid=self.param_grid)
                print(f"[Walkforward window {idx+1}] Best parameters: {best_params}")
            # --- Run signals on test_df with optimal params ---
            signals = strategy.generate_signals(test_df)
            test_df = test_df.reset_index(drop=True)
            signals = signals.reset_index(drop=True)

            # --- Simulate trades: one trade at a time, only one open ---
            trades, equity, win, loss, gross_profit, gross_loss, max_drawdown = \
                self.simulate_trades(test_df, signals, capital)
    ...
    def simulate_trades(self, df, signals, capital):
        """
        Simulate one trade at a time, entering on signal, exit at stop/target or end of test.
        Returns
            trades: list of dicts (entry, exit, pl, etc)
            equity_curve: list of capital after each trade
            win: count
            loss: count
            gross_profit: sum
            gross_loss: sum
        ...
        for i, row in df.iterrows():
            ...
            if not in_trade and sig["signal"] in ("BUY", "SELL"):
                ...
                # Simulate holding until stop or target or end of test
                exit_idx = None
                exit_price = None
                exit_reason = None
    ...
    def compute_sharpe(equity_curve):
        ...
        if rets.std() == 0:
            return 0
        sharpe = np.mean(rets) / np.std(rets) * np.sqrt(252)  # Assuming daily
        return sharpe
