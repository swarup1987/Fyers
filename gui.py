...
def open_backtest_window(parent):
    ...
    # Optional: Let user input (or just display) optimization grid
    grid_label = Label(opt_frame, text="(Using default parameter grid for optimization)")
    ...
    def on_backtest():
        ...
        try:
            ...
            show_backtest_results(win, stats, trades, per_window)
        except Exception as e:
...
def show_backtest_results(parent, stats, trades, per_window=None):
    ...
    results_box = Text(win, height=15, width=140, wrap='none', font=("Courier New", 10))
    ...
    # Show per-window optimization results if available
